...
class ScamDetector(gl.Contract):
    ...
    def _analyze(self, input_type, input_data):
        ...
        def leader_fn():
            # instruct the AI to return JSON directly
            response = gl.nondet.exec_prompt(prompt, response_format="json")
            return response

        def validator_fn(leader_res):
            # Check if leader returned a valid result
            ...
            # Compare classifications (must match for consensus)
            # We allow slight variation in confidence or reasoning as long as 
            # the core verdict (classification) is the same.
            return my_res["classification"] == leader_res.calldata["classification"]

        # Run with consensus
        result = gl.vm.run_nondet_unsafe(leader_fn, validator_fn)
        
        # Return as JSON string per GenLayer constraints
